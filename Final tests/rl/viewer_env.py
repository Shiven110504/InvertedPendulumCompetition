...
class ViewerEnvConfig:
    ...
    push_force_max: float = 1e6  # effectively uncapped
...
class ViewerResidualEnv:
    ...
    def reset(self) -> np.ndarray:
        ...
        if self.cfg.base_yaw_pos_noise > 0.0:
            base_idx = self.model.jnt_qposadr[mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_JOINT, "base_yaw")]
            self.data.qpos[base_idx] += self.rng.uniform(

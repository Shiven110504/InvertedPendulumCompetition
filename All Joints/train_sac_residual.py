...
TRAINING_PLAN = {
    "curriculum": [
        "Phase A: no pushes; ramp pend_vel/base_yaw noise upward while hitting a 2000-step horizon.",
        "Phase B: randomized pushes with timing jitter; push force resampled and escalated toward ~0.0105; horizon 5000.",
        "Phase C: train directly in viewer_env with Run_PendulumEnv-like pushes; long horizon (50k), fixed 4s gap.",
    ],
    "steps": 6_000_000,
    "random_steps": 25_000,
    "batch_size": 256,
...
REWARD_DESCRIPTION = {
    ...
    "yaw_rate_penalty": "0.25 * |yaw_dot|;",
    "tip_velocity_penalty": "0.1 * ||tip_vel_xy||",
    "action_penalty": "0.02 * ||a||^2",
    "survival_bonus": "+0.1 per step",
    "notes": "Base yaw position is omitted from observations and reward (rotational symmetry).",
}
...
def build_envs(args: argparse.Namespace) -> Dict[str, MiniArmResidualEnv]:
    ...
    viewer_train_env = ViewerResidualEnv(dataclasses.replace(viewer_cfg))
    viewer_env = ViewerResidualEnv(dataclasses.replace(viewer_cfg))
    env.rng = np.random.default_rng(args.seed)
    eval_env.rng = np.random.default_rng(args.seed + 123)
    viewer_train_env.rng = np.random.default_rng(args.seed + 456)
    viewer_env.rng = np.random.default_rng(args.seed + 999)
    return {"train": env, "eval": eval_env, "viewer_train": viewer_train_env, "viewer": viewer_env}
...
def main() -> None:
    ...
    env = envs["train"]
    eval_env = envs["eval"]
    viewer_train_env = envs["viewer_train"]
    viewer_env = envs["viewer"]

    active_env = env
    active_eval_env = eval_env
    ...
    phases = [
        ...
        },
        {
            ...
            "cfg": dict(
                ...
                base_yaw_pos_noise=0.0,
                push_force_min=0.0005,
                push_force_max=0.0015,  # goal upper bound
                push_force_start=0.0005,
        ...
        },
        {
            ...
            "noise_targets": {"pend_vel": 0.0, "base_yaw": 0.05},
            "noise_increment": {"pend_vel": 0.0, "base_yaw": 0.0},
    ...
    def apply_phase(phase_idx: int):
        nonlocal active_env, active_eval_env
        phase = phases[phase_idx]
        # Phase C trains directly in viewer env.
        if phase["name"] == "phase_C":
            active_env = viewer_train_env
            active_eval_env = viewer_env
        else:
            active_env = env
            active_eval_env = eval_env
        ...
        for k, v in phase["cfg"].items():
            setattr(active_env.cfg, k, v)
            setattr(active_eval_env.cfg, k, v)

        active_env.cfg.max_episode_steps = phase["cfg"]["max_episode_steps"]
        active_eval_env.cfg.max_episode_steps = phase["cfg"]["max_episode_steps"]
        active_env.cfg.pend_vel_noise = noise_state["pend_vel"]
        active_env.cfg.base_yaw_pos_noise = noise_state["base_yaw"]
        active_eval_env.cfg.pend_vel_noise = noise_state["pend_vel"]
        active_eval_env.cfg.base_yaw_pos_noise = noise_state["base_yaw"]
        ...
        if phase["name"] == "phase_B":
            ...
            active_env.cfg.push_force_max = phase["cfg"]["push_force_max"]
            active_eval_env.cfg.push_force_max = phase["cfg"]["push_force_max"]

        obs_local = active_env.reset()
        active_eval_env.reset()
    ...
    for step in range(1, args.total_steps + 1):
        if step <= args.random_steps:
            action = np.random.uniform(-1.0, 1.0, size=active_env.action_size())
        else:
        ...
        if step % args.eval_interval == 0:
            eval_stats = evaluate_policy(active_eval_env, agent, args.eval_episodes)
